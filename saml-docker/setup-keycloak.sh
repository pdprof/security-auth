...
$KCADM create clients/$CID/client-secret --server http://localhost:8080/auth --realm master --user $KEYCLOAK_USER --password $KEYCLOAK_PASSWORD
...
$KCADM update clients/$CID/optional-client-scopes/8995be17-d28b-474e-8e3e-fb57df19fc63 -r pdprof --server http://localhost:8080/auth --realm master --user $KEYCLOAK_USER --password $KEYCLOAK_PASSWORD
